...
realdata_set = {"B2C1", "Trial_Bellettini", "Trial_Fairwinds Brittany Park", "Trial_Empress Senior Living", "Trial_FTJ"}
...
for item, data in pivot_tables.items():
    ...
    for i in range(len(data)-1):
        ...
        max_value = data.max().sum()
        plt.ylim(bottom=0, top=max_value * 1.1)
        #data = data.drop(data.index[0])
        data = data[data.iloc[i+2:, :].columns[data.iloc[i+2:, :].notna().any()].tolist()]
        plt.legend().remove()
        plt.savefig('/Users/saccyann/Documents/Thinkie/Data_Visualization_Code/kendall_plot_extestusers/'+item+'_'+str(i)+'.png')
        plt.close() 
